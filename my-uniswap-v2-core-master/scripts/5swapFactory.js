...
const {SMALLROUTERABI,ERC20ABI,PAIRABI,ONLINEABI} = require("./ABI.js");
...
const OnlineContractWithWallet = new ethers.Contract(uniFactoryAddr, ONLINEABI, wallet);
...
/*
	...
	const amountOutMin = ethers.parseEther("0.5");
	const path = [erc20Addr, erc20AddrB]; 
	const to = wallet.address;
	const deadline = Math.floor(Date.now() / 1000) + 10 * 60; //当前时间+10分钟的时间戳

    let tx = await OnlineContractWithWallet.swapExactTokensForTokens(
    		amountIn,amountOutMin,path,to,deadline
    );
    await tx.wait();
*/
	console.log(await OnlineContractWithWallet.allPairsLength());
	console.log(await OnlineContractWithWallet.allPairsLength());
