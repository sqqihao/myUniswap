...
async function main() {
    ...
    const txA = await erc20ContractWithWallet.approve(smallRouter.target, ethers.parseEther("1000"));
    // 等待交易被确认
    await txA.wait();
    console.log("A Tokens approved!");

    console.log("B Approving tokens...");
   	txB = await erc20ContractWithWalletB.approve(smallRouter.target, ethers.parseEther("1000"));
    // 等待交易被确认
    await txB.wait();
    ...
    const token0Amount = ethers.parseEther("500");
    const token1Amount = ethers.parseEther("100");
    const deadline = Math.floor(Date.now() / 1000) + 10 * 60; //当前时间+10分钟的时间戳

    const smallRouterContract = new ethers.Contract(smallRouter.target, SMALLROUTERABI, wallet);

    let txPair = await smallRouterContract.getPairTest();
