...
async function addLiquidity() {    
    ...
    const txA = await tokenContractWithWallet.approve(smallRouter, ethers.parseEther("1000"));
    // 等待交易被确认
    await txA.wait();
    console.log("A Tokens approved!");

    console.log("B Approving tokens...");
    const txB = await tokenContractBWithWallet.approve(smallRouter, ethers.parseEther("50200000000000000000000"));
    // 等待交易被确认
    await txB.wait();
